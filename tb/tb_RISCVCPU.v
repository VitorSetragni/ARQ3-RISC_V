...
module tb_RISCVCPU;
    ...
    initial begin
        run_scheduled_no_dependencies();
        run_forwarding_and_stall();
        $finish;
    end
    ...
    task run_scheduled_no_dependencies;
        begin
            clear_memories();
            load_program_full_dependencies();
            reset_cpu();
            while(!halt) @(posedge clock);
            print_stats("scheduled_no_dependencies");
            check_expected();
            print_state_nonzero();
        end
    endtask

    task run_forwarding_and_stall;
        begin
            clear_memories();
            load_program_forwarding_dependencies();
            reset_cpu();
            while(!halt) @(posedge clock);
            print_stats("forwarding_and_stall");
            check_expected();
            print_state_nonzero();
        end
    endtask
    ...
    task load_program_full_dependencies;
        begin
            ...
            // Programa da Parte 2: dependências removidas por NOPs.
            // Neste modelo didático, os NOPs garantem que o consumidor leia
            // o registrador somente depois do write-back do produtor.
            ...
            cpu.IMemory[21] = 32'h0000000b; // halt
        end
    endtask

    task load_program_forwarding_dependencies;
        begin
            cpu.DMemory[0] = 32'd10;

            // Programa da Parte 3: dependências mantidas para validar
            // forwarding e stall load-use.
            cpu.IMemory[0] = 32'h00002083; // lw   x1, 0(x0)       # x1 = mem[0]
            cpu.IMemory[1] = 32'h00508113; // addi x2, x1, 5       # load-use: precisa stall
            cpu.IMemory[2] = 32'h00110193; // addi x3, x2, 1       # forwarding MEM/WB
            cpu.IMemory[3] = 32'h00302223; // sw   x3, 4(x0)       # forwarding para store
            cpu.IMemory[4] = 32'h00a18213; // addi x4, x3, 10      # forwarding
